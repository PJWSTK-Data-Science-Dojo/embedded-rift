...
def parse_match_result(match_result_json):
    parsed_result = {}
    metadata = {"matchId": match_result_json['metadata']['matchId'],
    ...
    result = {"gameStartTimestamp": match_result_json['info']['gameStartTimestamp'],
              "gameEndTimestamp": match_result_json['info']['gameEndTimestamp'],
              "gameDuration": match_result_json['info']['gameDuration']}

    participants = []
    keys_to_remove = ['challenges', 'missions', 'individualPosition', 'lane', 'role', 'teamPosition']
