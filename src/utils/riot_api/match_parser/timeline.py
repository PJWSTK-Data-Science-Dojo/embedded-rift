...
IGNORED = {
    2003,
    2031,
    2055,  # Health Potion, Refillable Potion, Control Ward
    2138,
    2139,
    2140,  # Elixir of Iron, Elixir of Sorcery, Elixir of Wrath
    3340,
    3363,
    3364,
...
SOULS = [
    ...
    "Hextech",
    "Chemtech",
]  # possible dragon souls, same names used by RiotAPI
...
def parse_timeline(timeline_json: dict) -> dict:
    ...
    for i, frame in enumerate(frames):
        ...
        frame_dict = {
            "teams": [{"eventData": init_team_event_data()} for _ in range(2)],
            "participants": [
        ...
        if i > 0:
            ...
            for team_id in range(2):
                frame_dict["teams"][team_id]["eventData"] = parsed_frames[i - 1][
                    "teams"
                ][team_id]["eventData"].copy()
            for participant_id in range(10):
                frame_dict["participants"][participant_id]["eventData"] = parsed_frames[
                    i - 1
                ]["participants"][participant_id]["eventData"].copy()
        ...
        for player_id in range(10):
            ...
            frame_dict["participants"][player_id] = {
                **frame_dict["participants"][player_id],
                **participantFrame,
            }
    ...
    for i in range(len(parsed_frames)):
        ...
        for team_id in range(2):
            parsed_frames[i]["teams"][team_id]["participants"] = participants[
                team_id : team_id + 5
            ]
...
def parse_champion_kill_event(frame_dict: dict, event: dict) -> dict:
    ...
    if "killerId" in event.keys():
        ...
        participants[killer_id]["eventData"]["kills"] = (
            participants[killer_id]["eventData"].setdefault("kills", 0) + 1
        )

    victim_id = event["victimId"] - 1
    participants[victim_id]["eventData"]["deaths"] = (
        participants[victim_id]["eventData"].setdefault("deaths", 0) + 1
    )
    assistant_ids = []
    ...
    for assistant_id in assistant_ids:
        participants[assistant_id]["eventData"]["assists"] = (
            participants[assistant_id]["eventData"].setdefault("assists", 0) + 1
        )
...
def parse_item_event(frame_dict, event):
    ...
    if event["type"] == "ITEM_PURCHASED":
        ...
        try:
            ...
            ValueError
        ):  # One of the ignored items or possibly an item received from runes
            pass
...
def parse_elite_monster_kill_event(frame_dict: dict, event: dict) -> dict:
    team_id = (
        0 if event["killerTeamId"] == Team.BLUE else 1
    )  # team_id - id of the team that killed the monster
    ...
    elite_monsters_killed = frame_dict["teams"][team_id]["eventData"][
        "eliteMonstersKilled"
    ]
...
def parse_dragon_soul_given_event(frame_dict: dict, event: dict) -> dict:
    team_id = (
        0 if event["teamId"] == Team.BLUE else 1
    )  # team_id - id of the team that gets the soul
...
def parse_building_kill_event(frame_dict: dict, event: dict) -> dict:
    ...
    team_id = (
        0 if event["teamId"] == Team.RED else 1
    )  # team_id - id of the team that destroyed the building

    if event["buildingType"] in BUILDINGS:
        building_id = BUILDINGS.index(event["buildingType"])  # Tower, Inhibitor
        frame_dict["teams"][team_id]["eventData"]["buildingsDestroyed"][
            BUILDINGS_LABELS[building_id]
        ] += 1
